...
1.  **Internal Override (if active):** If 'internal' is one of the **active** categories AND the command is a known SimShell internal command (like 'help', 'clear', 'add_int_cmd', 'list py vars', 'export db', etc., or a custom defined one), *always* classify it as 'internal', even if it might resemble another category.
...
- internal: SimShell specific commands like 'help', 'clear', 'mode', 'history', 'define', 'refine', 'add_int_cmd', 'export log', 'export db', 'pause', 'create sqlite', 'init', 'init db', 'list py vars', 'show requirements', 'persist memory db to', 'ai', any custom defined internal commands, AND variable assignments (e.g., 'x = 5', 'name = "test"').
