 ...
 newCustomCommands = internalResult.newCustomCommands; // Assign newCustomCommands from internalResult
      ...
      // --- Other Category Handlers ---
        ...
        logEntry = { timestamp, type: tsLogType, flag: tsLogFlag, text: `TypeScript simulation: ${simOutput}` };
         ...
         const errorMsg = `Error: Command not recognized within active categories. The command does not match any specific pattern for the active categories. It's not a variable assignment, an internal command, or a Python statement: ${commandTrimmed}`;
         ...
         logEntry = { timestamp, type: 'E', flag: 0, text: errorMsg };
       }
